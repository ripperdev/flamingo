...
public:
    ...
    int32_t getUserClientType() const {
        return m_userinfo.clienttype;
    }
    ...
    void checkHeartbeat(const std::shared_ptr<TcpConnection> &conn) const;
...
private:
    ...
    void
    onMoveFriendToOtherTeamResponse(int32_t friendid, const std::string &newteamname, const std::string &oldteamname,
                                    const std::shared_ptr<TcpConnection> &conn);

    void deleteFriend(const std::shared_ptr<TcpConnection> &conn, int32_t friendid);

    //根据用户分组信息组装应答给客户端的好友列表信息
    void makeUpFriendListInfo(std::string &friendinfo, const std::shared_ptr<TcpConnection> &conn) const;

    //将聊天消息的本地时间改成服务器时间，修改成功返回true,失败返回false。
    bool modifyChatMsgLocalTimeToServerTime(const std::string &chatInputJson, std::string &chatOutputJson) const;
