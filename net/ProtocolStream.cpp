...
namespace net {
    ...
    void write7BitEncoded(uint32_t value, std::string &buf) {
        do {
            auto c = (unsigned char) (value & 0x7F);
            value >>= 7;
            if (value)
                c |= 0x80;
    ...
    void BinaryStreamWriter::WriteCString(const char *str, size_t len) {
        std::string buf;
    ...
    void BinaryStreamWriter::WriteString(const string &str) {
        WriteCString(str.c_str(), str.length());
    }
